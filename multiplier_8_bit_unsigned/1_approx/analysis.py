############    MOLTIPLICATORE 8-BIT SENZA SEGNO CON APPROSSIMAZIONE 1  ############

#   Import del modulo C contenente i modelli di EvoApproxLib Lite
...
for i in range(0,op_len):
    for j in range(0,op_len):
        ...
        diff = abs(mul8u_LK8.mul(i,j) - (i*j))
        ...
        if i > 0 and j > 0:
            ...
            diff_rel = abs(mul8u_LK8.mul(i,j) - (i*j))/(i*j)
...
#   Visualizzazione a schermo dei valori delle metriche per il moltiplicatore
